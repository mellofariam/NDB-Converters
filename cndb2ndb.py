...
parser.add_argument('-f', metavar='input-file-CNDB-frames',help='cndb file', action='store', dest='arg_fname', required=True)
...
### [ HEADER ]
ndbf.write(header_string.format('HEADER','NDB File generated by Open-MiChroM'," ", " "))
ndbf.write(title_string.format('TITLE ','  ','A Scalable Computational Approach for '))
ndbf.write(title_string.format('TITLE ','2 ','Simulating Complexes of Multiple Chromosomes'))
...
Seqlist = [Type_conversion[x] for x in type_list]
Seqlista = chunks(Seqlist,23)

for num, line in enumerate(Seqlista):
    ndbf.write(seqchr_string.format("SEQCHR", num+1, "C1", 
                                     len(type_list)," ".join(line)))
...
for mdl in range(1,totalModel,1):
    ...
    ndbf.write(ter_string.format('TER', len(data)+1, Seqlist[-1], 'C1'))
    ndbf.write('\nENDMDL\n')
    model += 1
